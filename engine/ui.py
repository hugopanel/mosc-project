...
class TreeToolTip(UIComponent):
    def __init__(self, x, y, width, height):
        super().__init__(x, y, width, height)
        
    def draw(self, screen):
        row1 = engine.config.font_heading.render("(0, 1) Tree [ABAC]", True, "white")
        row2 = engine.config.font_small.render("Status: sick [AC]", True, "gray")
        row3 = engine.config.font_small.render("Age: 3 cycles", True, "gray")
        row4 = engine.config.font_small.render("Production: +5/cycle blfglad gfa;jg asd;kfj dslkajf asldkj ", True, "gray")
        ...
        if position[1] + box_height > engine.config.screen_height:
...
class GardenTile(pygame.sprite.Sprite):
    ...
    def draw(self):
        ...
        image.set_colorkey(pygame.Color(100, 100, 0, 0), pygame.RLEACCEL)
        return image
...
class Garden:
    def __init__(self, graph, size_x, size_y, position_x, position_y):
        self.graph = graph
        self.size_x = size_x
        self.size_y = size_y
        ...
        for y in range(size_y):
            ...
            for _ in range(size_x):
                self.garden_tiles[y].append(GardenTile(os.path.join("assets", "Tileset.png"), 0, 16))
